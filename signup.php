...
<?php
    if (isset($_POST['submit'])) 
    {
        ...
        $folder ="customer_img/".$image_name;

        $query = "SELECT * FROM customers WHERE c_email='$em'";
        ...
        elseif ($result==0) 
        {
            $query = "INSERT INTO customers (c_name,c_email,c_password,c_country,c_city,c_contact,c_address,c_img,c_ip) VALUES('$na','$em','$pa','$co','$ci','$mo','$ad','$folder','$ip')";
            $data = mysqli_query($conn ,$query);
            move_uploaded_file($temp_name, $folder);
            if ($data) 
            {
                echo "<script>alert('Signup successfully, Now login!')</script>";
